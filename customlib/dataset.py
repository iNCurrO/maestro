...
def set_dataset(config):
    ...
    assert ds.shape()[3] == config.num_det and ds.shape()[2] == config.view, f"Error! the shape of sinogram data ({ds.shape}) is different from the setting ({config.det_num}, {config.view_num})"
