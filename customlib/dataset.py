...
def set_dataset(config):
    ...
    assert(
        ds.shape()[0] == config.num_det and ds.shape()[1] == config.view,
           "Error! the shape of sinogram data ({ds.shape}) is different from the setting ({config.det_num}, {config.view_num})"
           )
